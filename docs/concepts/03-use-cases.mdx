...
therefore are a great fit for Vesting.
...
Instant airdrops is the traditional way of running airdrops where tokens have no vesting and can instantly be claimed by
the users. However, running an airdrop campaign can become very expensive if you are storing airdrop data on-chain.

This is where Merkle Airdrops come into the picture. When you run a campaign with Sablier, the airdrops information is
stored in a merkle tree, hosted on IPFS. The EVM storage only stores the root of the Merkle tree. At the time of claim,
eligible users can provide merkle proof of their claims which is verified on-chain.
